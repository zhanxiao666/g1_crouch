...
struct ArticulationData
{
    ...
    // Current commanded base height. Observation terms can use this to keep
    // externally controlled limbs out of the policy feedback loop.
    float target_height_cmd = 0.76f;

    // Root angular velocity in base world frame.
    Eigen::Vector3f root_ang_vel_b;

    // Root linear velocity in base frame.
    Eigen::Vector3f root_lin_vel_b;
