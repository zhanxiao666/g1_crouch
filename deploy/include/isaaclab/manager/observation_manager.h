...
{
public:
    ObservationManager(YAML::Node cfg, ManagerBasedRLEnv* env, YAML::Node noise_cfg = YAML::Node())
    :cfg(cfg), env(env)
    {
        try {
            deploy_noise_enabled = noise_cfg["enabled"].as<bool>(false);
        } catch(const std::exception&) {
            deploy_noise_enabled = false;
        }
        _prapare_terms();
    }
...
protected:
    ...
    std::vector<ObservationTermCfg> _prepare_group_terms(const YAML::Node & group_cfg)
    {
        ...
        for(auto it = group_cfg.begin(); it != group_cfg.end(); ++it)
        {
            ...
            term_cfg.noise_enabled = deploy_noise_enabled;
            ...
            if(term_yaml_cfg["noise_uniform"].IsDefined() && !term_yaml_cfg["noise_uniform"].IsNull()) {
                const auto noise = term_yaml_cfg["noise_uniform"].as<std::vector<float>>();
                auto obs = term_cfg.func(this->env, term_cfg.params);
                if(noise.size() == 2) {
                    term_cfg.noise_uniform_min.assign(obs.size(), noise[0]);
                    term_cfg.noise_uniform_max.assign(obs.size(), noise[1]);
                } else if(noise.size() == obs.size()) {
                    term_cfg.noise_uniform_min.resize(obs.size());
                    term_cfg.noise_uniform_max.resize(obs.size());
                    for(size_t i = 0; i < obs.size(); ++i) {
                        term_cfg.noise_uniform_min[i] = -std::abs(noise[i]);
                        term_cfg.noise_uniform_max[i] = std::abs(noise[i]);
                    }
                } else {
                    throw std::runtime_error("Observation term '" + term_name + "' has invalid noise_uniform size.");
                }
            }
    ...
    bool deploy_noise_enabled = false;
