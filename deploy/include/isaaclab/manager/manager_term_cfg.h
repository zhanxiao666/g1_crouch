...
struct ObservationTermCfg
{
    ...
    std::vector<float> noise_uniform_min;
    std::vector<float> noise_uniform_max;
    ...
    void add(std::vector<float> obs)
    {
        ...
        {
            if(noise_enabled && !noise_uniform_min.empty() && !noise_uniform_max.empty()) {
                static thread_local std::mt19937 rng(std::random_device{}());
                std::uniform_real_distribution<float> dist(noise_uniform_min[j], noise_uniform_max[j]);
                obs[j] += dist(rng);
            }
            if(scale_first) {
